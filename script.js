let API_KEY = 'AIzaSyAPCj_2telghy5Cu2nMa9Q7Cyx18QptNzQ';

const videoCardContainer = document.querySelector('.videos_container');
let video_url ='https://youtube.googleapis.com/youtube/v3/videos?part=snippet&chart=mostPopular&maxResults=100&regionCode=IN&key=' +
     API_KEY;
...
form.addEventListener('submit', e => {
  ...
  if (searchValue) {
    getVideos(
      `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q=${searchValue}&key=` + API_KEY
    );
  } else {
    getVideos(video_url);
  }
